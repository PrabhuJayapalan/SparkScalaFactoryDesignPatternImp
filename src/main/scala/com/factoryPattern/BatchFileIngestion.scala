...
import com.BatchLoadImplementation.{AvroIngestion, JsonIngestion, ORCIngestion, XMLIngestion}
...
object BatchFileIngestion {
  ...
  def apply(fileType : String, filePath : String, outputPath: String)= {
    ...
    constantObject match {
      ...
      case ConstantsImmut(filePath, outputPath, Some(GenericConfiguration.FILE_TYPE_AVRO), None) =>new AvroIngestion(filePath : String, outputPath : String).dataIngestion
      case ConstantsImmut(filePath, outputPath, Some(GenericConfiguration.FILE_TYPE_ORC), None) =>new ORCIngestion(filePath : String, outputPath : String).dataIngestion
